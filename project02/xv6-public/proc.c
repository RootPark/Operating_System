...
//function to findout their is even function exist or not
int check_even(void){
	struct proc* p;
	int count=0;
	for(p = ptable.proc; p<&ptable.proc[NPROC];p++){
		if((p->pid) % 2 == 0 && (p->pid) !=2 && (p->state) == RUNNABLE){
			count++;
		}
	}
	if(count == 0){
...
void scheduler(void){
	...
	for(;;){
		...
		for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
			...
			//cprintf("pid : %d, priority : %d, level : %d\n", proc_to_run->pid, proc_to_run->priority, proc_to_run->level);
...
void scheduler(void){
	...
	for(;;){
		...
		for(p=ptable.proc;p<&ptable.proc[NPROC];p++){
			...
			//Process is done ruuning for now.
			//It should have changed its p->state before coming back.
			c->proc= 0;
		}
		release(&ptable.lock);
...
int check_queue_level(void){
	struct proc* p;
	int max_level = 4;
	int count=0;
	for(p=ptable.proc; p<&ptable.proc[NPROC];p++){
		if((p->state) == RUNNABLE && (p->level) < max_level
				&& (p->pid) > 2){
			max_level = p->level;
			count++;
		}
...
struct proc* high_priority(int q_level){
	struct proc* p;
	struct proc* highest=0;
	int max=0;

	for(p=ptable.proc;p<&ptable.proc[NPROC];p++){
		if(p->priority > max && p->state == RUNNABLE
				&& p->level == q_level && p->pid>2){
			max = p->priority;
			highest = p;
		}
	}
